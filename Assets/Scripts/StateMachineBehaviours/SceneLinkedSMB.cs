...
namespace CVXIV {
    public class SceneLinkedSMB<TMonoBehaviour> : SealedSMB where TMonoBehaviour : MonoBehaviour {
        ...
        public static void Initialise(Animator animator, TMonoBehaviour monoBehaviour) {
            SceneLinkedSMB<TMonoBehaviour>[] monoBehaviours = animator.GetBehaviours<SceneLinkedSMB<TMonoBehaviour>>();
            foreach (var behaviour in monoBehaviours) {
                behaviour.InternalInitialise(animator, monoBehaviour);
            }
        }
        ...
        public sealed override void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex, AnimatorControllerPlayable controller) {
            ...
            // 正在进入这个state；GetNextAnimatorStateInfo和当前的stateInfo一致，且在IsInTransition，表明正在进入当前的stateInfo
            ...
            // 这里能够触发的条件是即将进入下一个state，也就是进入下一个state的第一帧；
            // 因为firstFrameHappened已经触发，说明当前state已经转变完成；而此时IsInTransition又为true，说明即将进入下一个状态
            ...
            // 正在离开这个state；GetCurrentAnimatorStateInfo和当前的state一致，且又IsInTransition，表明正在离开这个state
